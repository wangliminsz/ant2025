...
export class ContactService {
      ...
      const fetchData = () => {
        axios.get(dataURL, {
          ...
          if (newOffset) {
            ...
            dataURL = `${this.serverAirURL}/thai_notify?filterByFormula={user_id}='${userId}'`;
            fetchData();
          } else {
            resolve(allRecords);
          }
  ...
  static async getCondo(contactId, userId) {
    ...
    let dataURL = `${this.serverAirURL}/${this.table_name}?filterByFormula=AND({id}=${contactId}, {user_id}='${userId}', {record_del_flag}='no')`
      ...
      table.select({ filterByFormula: `{id} = ${contactId}` }).firstPage((err, records) => {
        ...
        if (records.length > 0) {
          ...
          table.update(airId, recordObj, (err, record) => {
            ...
            console.log(`Delete Airtable record ID for ID ${contactId} is ${record.id}`);
            // resolve(record.id);
            resolve('deleted');
          });
        } else {
      ...
      table.select({ filterByFormula: `{id} = ${contactId}` }).firstPage((err, records) => {
        ...
        if (records.length > 0) {
          ...
          table.update(airId, recordObj.fields, (err, record) => {
            ...
            console.log(`Update Airtable record ID for ID ${contactId} is ${record.id}`);
            resolve(record.id);
            // resolve('updated');
          });
        } else {
